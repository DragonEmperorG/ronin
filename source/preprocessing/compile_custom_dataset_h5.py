...
_device_list = ['asus1', 'asus2', 'asus3', 'pixel', 'samsung1', 'samsung2', 'Samsung']
_nano_to_sec = 1e09
_micro_to_nano = 1000
...
def compile_unannotated_sequence(root_dir, data_list, args):
    ...
    for data in data_list:
        try:
            ...
            with open(osp.join(data_path, 'date.txt')) as f:
                line = f.readline().split()
                date = line[0]

            with open(osp.join(data_path, 'device.txt')) as f:
                line = f.readline().split()
                if line[-1] in _device_list:
                    device = line[-1]
            ...
            json.dump(meta_info, open(osp.join(out_path, 'info.json'), 'w'), indent=2)
